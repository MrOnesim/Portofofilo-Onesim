...
type TabId = "about" | "projects" | "skills" | "experiences" | "blog" | "contact";
...
export const ListView: React.FC<ListViewProps> = ({ onSwitchToGame, onOpenProject }) => {
  const [darkMode, setDarkMode] = useState(() => {
    if (typeof window !== "undefined") return localStorage.getItem("onesim_theme") === "dark";
    return false;
  });
  ...
  const renderAbout = () => (
    <div className="max-w-4xl mx-auto space-y-8">
      {/* Hero Card */}
      <motion.div variants={fadeUp} className={`${card} p-8 md:p-10 rounded-2xl border ${border}`}>
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-8">
          <div>
            ...
            <h1 className={`text-4xl sm:text-5xl font-serif font-black tracking-tight ${textPrimary}`}>
              <TypewriterText text="Graça Onesim Géraldo Ema-ayé" />
            </h1>
            <p className={`text-base md:text-lg ${textSecondary} font-medium mt-2 max-w-xl leading-relaxed`}>
        ...
        {/* About text */}
        <div className={`mt-8 space-y-4 ${textBody} leading-relaxed`}>
  ...
  const renderProjects = () => (
    <div className="max-w-6xl mx-auto">
      <motion.div variants={fadeUp} className="mb-8">
        <h2 className={`text-3xl font-serif font-black ${textPrimary} flex items-center gap-2 mb-6`}>
          <Sparkles className="w-7 h-7 text-[#3b82f6]" />
          Projets &amp; Réalisations
        </h2>
        <div className={`flex flex-wrap gap-1 ${categoryBg} p-1 rounded-xl w-fit mb-6`}>
          {[
            { label: "Tous", key: "Tous" },
        ...
        <div className="relative mb-4">
          ...
          <input type="text" placeholder="Rechercher un projet, une technologie..." value={searchTerm}
        ...
        <div className="flex flex-wrap gap-1.5 mb-6">
          {[...new Set(PROJECTS_DATA.flatMap(p => p.technologies))].sort().slice(0, 15).map(tech => (
            <button key={tech} onClick={() => { sound.playClick(); setSearchTerm(tech); }}
              className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${searchTerm === tech ? "bg-[#3b82f6] text-white shadow-sm" : `${isDark ? "bg-gray-800 text-gray-400 hover:bg-gray-700" : "bg-[#ebdccb]/30 text-[#6b584a] hover:bg-[#ebdccb]/60"}`}`}>{tech}</button>
          ))}
        </div>
      </motion.div>
      ...
      {filteredProjects.length > 0 ? (
        <motion.div variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.map((project) => (
            <motion.div key={project.id} variants={fadeUp} onClick={() => handleProjectClick(project)}
              ...
              <div className="relative aspect-video overflow-hidden">
                <img src={project.images[0]} alt={`Capture d'écran du projet ${project.title}`} className="w-full h-full object-cover" loading="lazy"
                  onError={(e) => {
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`text-center py-16 ${card} rounded-2xl border border-dashed ${border}`}>
  ...
  const renderSkills = () => (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      {/* Certifications */}
      <motion.div variants={fadeUp} className={`${card} p-8 rounded-2xl border ${border}`}>
        <h2 className={`text-2xl font-serif font-black ${textPrimary} mb-6 flex items-center gap-2 pb-2 border-b ${border}`}>
          <Award className="w-6 h-6 text-[#3b82f6]" />
          Certifications
        </h2>
        <div className="flex flex-wrap gap-2">
          {["OpenAI", "Google", "freeCodeCamp", "Meta", "Microsoft"].map((cert) => (
            <span key={cert} className={`px-3 py-1.5 ${tagBg} rounded-lg text-xs font-bold border ${borderLight} transition-colors cursor-default`}>{cert}</span>
          ))}
    ...
    <div className="max-w-4xl mx-auto">
      <motion.div variants={fadeUp} className={`${card} p-8 rounded-2xl border ${border}`}>
        ...
        <div className="space-y-4">
          {BLOG_POSTS.map((post) => (
            <motion.div key={post.id} whileHover={{ x: 4 }} onClick={() => { sound.playClick(); setSelectedPost(post); }}
              className={`group cursor-pointer p-4 rounded-xl ${isDark ? "hover:bg-gray-800/50" : "hover:bg-[#f0ebdf]/50"} transition-colors -mx-3`}>
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <motion.div variants={fadeUp} className={`${card} p-8 rounded-2xl border ${border}`}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <a href={SOCIAL_LINKS.facebook} target="_blank" rel="noreferrer" onClick={() => sound.playClick()}
            className={`flex items-center justify-center gap-2 ${sectionBg} ${isDark ? "hover:bg-gray-700 text-gray-300" : "hover:bg-[#ebdccb] text-[#2c2621]"} py-2.5 rounded-xl text-xs font-bold transition-all border border-transparent ${isDark ? "hover:border-gray-700" : "hover:border-[#ebdccb]"}`}>
  ...
  return (
    <div className={`min-h-screen ${bg} font-sans selection:bg-[#3b82f6] selection:text-white transition-colors duration-300 flex flex-col`}>
      ...
      <nav className={`sticky top-0 z-40 w-full backdrop-blur-md border-b ${border} ${isDark ? "bg-gray-950/90" : "bg-[#f7f2ea]/90"} transition-all duration-300`}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button onClick={() => switchTab("about")}
            className={`font-serif font-black tracking-tight text-xl ${textPrimary} hover:text-[#3b82f6] transition-colors cursor-pointer shrink-0`}>
            ONESIM
          </button>
              ...
              <button key={tab.id} onClick={() => switchTab(tab.id)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
                  activeTab === tab.id
                    ...
                    : `${textSecondary} hover:text-[#3b82f6] hover:bg-white/5`
                }`}>
                {tab.label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <button onClick={toggleDark}
              className={`p-2 rounded-lg border ${border} ${isDark ? "hover:bg-gray-800 text-gray-300" : "hover:bg-white text-[#2c2621]"} transition-all text-xs font-bold`}
            ...
            <button onClick={() => { sound.playClick(); onSwitchToGame(); }}
            ...
            {/* Mobile hamburger */}
            <button onClick={() => { sound.playClick(); setMobileMenuOpen(!mobileMenuOpen); }}
              className={`md:hidden p-2 rounded-lg border ${border} ${isDark ? "hover:bg-gray-800 text-gray-300" : "hover:bg-white text-[#2c2621]"} transition-all`}>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              className={`md:hidden overflow-hidden border-t ${border}`}>
                  ...
                  <button key={tab.id} onClick={() => switchTab(tab.id)}
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-all ${
                      activeTab === tab.id
                        ...
                        : `${textSecondary} hover:text-[#3b82f6]`
                    }`}>
                    {tab.label}
                  </button>
                ))}
      ...
      <main className="flex-1 px-4 py-10">
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} {...pageTransition} className="max-w-6xl mx-auto">
            {renderPage()}
          </motion.div>
        </AnimatePresence>
      </main>
